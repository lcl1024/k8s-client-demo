...
func PackageGoLangCC(chaincodePath string) ([]byte, error) {
	...
	wd, _ := os.Getwd()
	goPath := path.Join(wd, beego.AppConfig.String("ILIOS"), "chaincode")
	if goPath == "" {
	...
	projDir := path.Join(goPath, "src/publicchannel_cc", chaincodePath)
	...
	//ioutil.WriteFile("test1.tar", tarBytes, 0644)
	if err != nil {
		return nil, err
	}
	return tarBytes, nil

...
var chcfgOpts = make(map[string][]chconfig.Option)
...
func (c *contextCache) getChCfgOpts(channelID string) ([]chconfig.Option, error) {
	if opts, ok := chcfgOpts[channelID]; ok {
		return opts, nil
	}

	options := []chconfig.Option{}
	querychannelCfg := c.ctx.EndpointConfig().ChannelConfig(channelID).Policies.QueryChannelConfig
	orderercfg, exist, ignoreOrderer := c.ctx.EndpointConfig().OrdererConfig(querychannelCfg.Orderer)
	if ignoreOrderer {
		return nil, errors.Errorf("orderer url : %s is explicitly ignored by EntityMatchers config - can't add orderer", channelID)
	}
	if exist {
		orderer, err := c.ctx.InfraProvider().CreateOrdererFromConfig(orderercfg)
		if err != nil {
			return nil, err
		}
		options = append(options, chconfig.WithOrderer(orderer))
	}

	peers := []fab.Peer{}
	for _, peer := range querychannelCfg.Targets {
		peercfg, exist := c.ctx.EndpointConfig().PeerConfig(peer)
		if exist {
			networkPeer := &fab.NetworkPeer{PeerConfig: *peercfg, MSPID: c.ctx.Identifier().MSPID}
			if exist {
				peer, err := c.ctx.InfraProvider().CreatePeerFromConfig(networkPeer)
				if err != nil {
					return nil, err
				}
	...
	if len(peers) != 0 {
		options = append(options, chconfig.WithPeers(peers))
	}
	chcfgOpts[channelID] = options
	return options, nil
}

func (c *contextCache) loadChannelCfgRef(channelID string) (*chconfig.Ref, error) {
	opts, err := c.getChCfgOpts(channelID)
	if err != nil {
		return nil, err
	}

	key, err := chconfig.NewCacheKey(c.ctx, func(string) (fab.ChannelConfig, error) { return chconfig.New(channelID, opts...) }, channelID)
	if err != nil {
		return nil, err
	}

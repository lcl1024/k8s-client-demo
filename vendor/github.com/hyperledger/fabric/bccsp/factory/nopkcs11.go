	...
	//factoriesInitOnce.Do(func() {
		...
		if config.SwOpts != nil {
			...
			case "GM":
				...
				x509.InitX509(config.SwOpts.Library)
				err := gm.InitGMPlugin(config.SwOpts.Library)
				if err != nil {
					factoriesInitError = errors.Wrapf(errors.Errorf("unrecognized gm plugin type: %s", config.SwOpts.Library), "Failed initializing BCCSP.")
				}
			default:

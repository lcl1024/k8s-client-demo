...
func ResetBlockStore(blockstorePath string, blkStoreConf *fsblkstorage.Conf) error {
	return fsblkstorage.ResetBlockStore(blockstorePath, blkStoreConf)
}

// ValidateRollbackParams performs necessary validation on the input given for
// the rollback operation.
func ValidateRollbackParams(blockstorePath, ledgerID string, blockNum uint64, blkStoreConf *fsblkstorage.Conf) error {
	return fsblkstorage.ValidateRollbackParams(blockstorePath, ledgerID, blockNum, blkStoreConf)
}

// Rollback reverts changes made to the block store beyond a given block number.
func Rollback(blockstorePath, ledgerID string, blockNum uint64, blkStoreConf *fsblkstorage.Conf) error {
	indexConfig := &blkstorage.IndexConfig{AttrsToIndex: attrsToIndex}
	return fsblkstorage.Rollback(blockstorePath, ledgerID, blockNum, indexConfig, blkStoreConf)
}

...
func binarySearchFileNumForBlock(rootDir string, blockNum uint64, blkStoreConf *Conf) (int, error) {
	cpInfo, err := constructCheckpointInfoFromBlockFiles(rootDir, blkStoreConf)
	if err != nil {
		return -1, err
	}
	...
	for endFile != beginFile {
		...
		n, err := retriveFirstBlockNumFromFile(rootDir, searchFile, blkStoreConf)
		if err != nil {
			return -1, err
		}
		switch {
...
func retriveFirstBlockNumFromFile(rootDir string, fileNum int, blkStoreConf *Conf) (uint64, error) {
	s, err := newBlockfileStream(rootDir, blkStoreConf, fileNum, 0)
	if err != nil {
		return 0, err
	}
	defer s.close()

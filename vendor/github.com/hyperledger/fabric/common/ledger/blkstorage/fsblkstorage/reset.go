...
func ResetBlockStore(blockStorageDir string, blkStoreConf *Conf) error {
	...
	for _, ledgerID := range ledgerIDs {
		...
		if err := recordHeightIfGreaterThanPreviousRecording(ledgerDir, blkStoreConf); err != nil {
			return err
		}
		if err := resetToGenesisBlk(ledgerDir, blkStoreConf); err != nil {
			return err
		}
	}
	return nil
...
func resetToGenesisBlk(ledgerDir string, blkStoreConf *Conf) error {
	...
	zeroFilePath, genesisBlkEndOffset, err := retrieveGenesisBlkOffsetAndMakeACopy(ledgerDir, blkStoreConf)
	if err != nil {
		return err
	}
	for lastFileNum > 0 {
...
func retrieveGenesisBlkOffsetAndMakeACopy(ledgerDir string, blkStoreConf *Conf) (string, int64, error) {
	blockfilePath := deriveBlockfilePath(ledgerDir, 0)
	blockfileStream, err := newBlockfileStream(ledgerDir, blkStoreConf, 0, 0)
	if err != nil {
		return "", -1, err
	}
...
func recordHeightIfGreaterThanPreviousRecording(ledgerDir string, blkStoreConf *Conf) error {
	logger.Infof("Preparing to record current height for ledger at [%s]", ledgerDir)
	checkpointInfo, err := constructCheckpointInfoFromBlockFiles(ledgerDir, blkStoreConf)
	if err != nil {
		return err
	}

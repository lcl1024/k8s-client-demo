...
func Rollback(blockStorageDir, ledgerID string, targetBlockNum uint64, indexConfig *blkstorage.IndexConfig, blkStoreConf *Conf) error {
	...
	if err := recordHeightIfGreaterThanPreviousRecording(r.ledgerDir, blkStoreConf); err != nil {
		return err
	}

	logger.Infof("Rolling back block index to block number [%d]", targetBlockNum)
	if err := r.rollbackBlockIndex(blkStoreConf); err != nil {
		return err
	}

	logger.Infof("Rolling back block files to block number [%d]", targetBlockNum)
	if err := r.rollbackBlockFiles(blkStoreConf); err != nil {
		return err
	}
...
func (r *rollbackMgr) rollbackBlockIndex(blkStoreConf *Conf) error {
	...
	for end >= start {
		...
		if err := r.deleteIndexEntriesRange(start, end, blkStoreConf); err != nil {
			return err
		}
...
func (r *rollbackMgr) deleteIndexEntriesRange(startBlkNum, endBlkNum uint64, blkStoreConf *Conf) error {
	...
	stream, err := newBlockStream(r.ledgerDir, blkStoreConf, lp.fileSuffixNum, int64(lp.offset), -1)
	defer stream.close()
...
func (r *rollbackMgr) rollbackBlockFiles(blkStoreConf *Conf) error {
	...
	targetFileNum, err := binarySearchFileNumForBlock(r.ledgerDir, r.targetBlockNum, blkStoreConf)
	if err != nil {
		return err
	}
	...
	endOffset, err := calculateEndOffSet(r.ledgerDir, targetFileNum, r.targetBlockNum, blkStoreConf)
	if err != nil {
		return err
	}
...
func calculateEndOffSet(ledgerDir string, targetBlkFileNum int, blockNum uint64, blkStoreConf *Conf) (int64, error) {
	stream, err := newBlockfileStream(ledgerDir, blkStoreConf, targetBlkFileNum, 0)
	if err != nil {
		return 0, err
	}
	defer stream.close()
...
func ValidateRollbackParams(blockStorageDir, ledgerID string, targetBlockNum uint64, blkStoreConf *Conf) error {
	...
	if err := validateTargetBlkNum(ledgerDir, targetBlockNum, blkStoreConf); err != nil {
		return err
	}
	return nil
...
func validateTargetBlkNum(ledgerDir string, targetBlockNum uint64, blkStoreConf *Conf) error {
	logger.Debugf("Validating the given block number [%d] agains the ledger block height", targetBlockNum)
	cpInfo, err := constructCheckpointInfoFromBlockFiles(ledgerDir, blkStoreConf)
	if err != nil {
		return err
	}

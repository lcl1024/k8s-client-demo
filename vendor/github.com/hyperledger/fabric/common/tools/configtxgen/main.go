...
func main() {
	var outputBlock, outputChannelCreateTx, channelCreateTxBaseProfile, profile, configPath, channelID, inspectBlock, inspectChannelCreateTx, outputAnchorPeersUpdate, asOrg, printOrg, bccsp, gmPlugin string
	...
	flag.StringVar(&bccsp, "bccsp", "SW", "Default specifies the preferred blockchain crypto service provider to use. If the preferred provider is not available, the software based provider (SW) will be used.")
	flag.StringVar(&gmPlugin, "gm", "gmsm", "\"The gm plugin to use. Default is gmsm.")
	...
	logger.Info("Bccsp ProviderName: ", bccsp)
	// 支持国密

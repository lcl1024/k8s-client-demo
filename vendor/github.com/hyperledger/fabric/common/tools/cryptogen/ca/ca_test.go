...
func TestLoadCertificateECDSA(t *testing.T) {
	...
	priv, _, err := csp.GeneratePrivateKey(certDir, "SW")
	...
	// create our CA
	rootCA, err := ca.NewCA(caDir, testCA3Name, testCA3Name, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, "SW")
	assert.NoError(t, err, "Error generating CA")

	cert, err := rootCA.SignCertificate(certDir, testName3, 0,nil, nil, ecPubKey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny}, "SW")
	assert.NoError(t, err, "Failed to generate signed certificate")
...
func TestNewCA(t *testing.T) {
	...
	caDir := filepath.Join(testDir, "ca")
	rootCA, err := ca.NewCA(caDir, testCAName, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, "SW")
	assert.NoError(t, err, "Error generating CA")
	...
	err = X509.CertCheckSignature(cert2, cert2.SignatureAlgorithm, cert2.RawTBSCertificate, cert2.Signature)
	assert.NoError(t, err)
...
func TestGenerateSignCertificate(t *testing.T) {
	...
	// create our CA
	rootCA, err := ca.NewCA(caDir, testCA2Name, testCA2Name, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, "SW")
	assert.NoError(t, err, "Error generating CA")

	cert, err := rootCA.SignCertificate(certDir, testName, 0, nil, nil, ecPubKey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny}, "SW")
	assert.NoError(t, err, "Failed to generate signed certificate")
	// KeyUsage should be x509.KeyUsageDigitalSignature | x509.KeyUsageKeyEncipherment
	...
	cert, err = rootCA.SignCertificate(certDir, testName, 0, nil, nil, ecPubKey,
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{}, "SW")
	assert.NoError(t, err, "Failed to generate signed certificate")
	assert.Equal(t, 0, len(cert.ExtKeyUsage))

	// make sure ous are correctly set
	ous := []string{"TestOU", "PeerOU"}
	cert, err = rootCA.SignCertificate(certDir, testName, ous, 0, nil, ecPubKey,
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{}, "SW")
	...
	cert, err = rootCA.SignCertificate(certDir, testName, 0, nil, sans, ecPubKey,
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{}, "SW")
	...
	_, err = rootCA.SignCertificate(certDir, "empty/CA", 0, nil, nil, ecPubKey,
		x509.KeyUsageKeyEncipherment, []x509.ExtKeyUsage{x509.ExtKeyUsageAny}, "SW")
	...
	_, err = badCA.SignCertificate(certDir, testName, 0, nil, nil, &crypto.PublicKey{},
		x509.KeyUsageKeyEncipherment, []x509.ExtKeyUsage{x509.ExtKeyUsageAny}, "SW")

...
// baseDir/name
func NewCA(baseDir, org, name, country, province, locality, orgUnit, streetAddress, postalCode string, bccspType string) (*CA, error) {
...
func (ca *CA) SignCertificate(baseDir, name string, nodeType int, ous, sans []string, pub interface{},
	ku x509.KeyUsage, eku []x509.ExtKeyUsage, bccsp string) (*x509.Certificate, error) {
	...
	// client and admin: set expiry to around 1 years
	if nodeType == 0 || nodeType == 3 {
		//	expiry := 365 * 24 * time.Hour
		//	template.NotAfter = template.NotBefore.Add(expiry).UTC()
	}
...
func LoadCertificateECDSA(certPath string) (*x509.Certificate, error) {
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		if strings.HasSuffix(path, ".pem") {
			...
			c, e := utils.DERToX509Certificate(block.Bytes)
			cert, err = (*x509.Certificate)(unsafe.Pointer(c)), e
		}
		return nil
	}

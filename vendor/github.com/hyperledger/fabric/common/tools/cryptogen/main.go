...
var (
	...
	genX509Plugin = gen.Flag("x509", "The x509 plugin to use. Default is standard x509").String()
	genGMPlugin   = gen.Flag("gm", "The gm plugin to use. Default is gmsm").String()
	bccsp         = gen.Flag("bccsp", "Specifies the preferred blockchain crypto service provider to use. Default use software based provider (SW)").String()
	...
	extX509Plugin = ext.Flag("x509", "The x509 plugin to use. Default is standard x509").String()
	extGMPlugin   = ext.Flag("gm", "The gm plugin to use. Default is gmsm").String()
	extBccsp      = ext.Flag("bccsp", "Specifies the preferred blockchain crypto service provider to use. Default use software based provider (SW)").String()
)
...
func extend() {
	...
	*bccsp = strings.ToUpper(*extBccsp)
	fmt.Printf("Bccsp ProviderName: %s\n", *bccsp)
	...
	// Load x509 plugin
	if err := x509.InitX509(*extX509Plugin); err != nil {
		fmt.Printf("Load x509 pulgin error: %s\n", err)
		os.Exit(1)
	}
...
func extendPeerOrg(orgSpec OrgSpec) {
	...
	generateNodes(peersDir, orgSpec.Specs, signCA, msp.PEER, orgSpec.EnableNodeOUs)
	...
	generateNodes(usersDir, users, signCA, msp.CLIENT, orgSpec.EnableNodeOUs)
}

func extendPeerOrgTLS(orgSpec OrgSpec) {
	orgName := orgSpec.Domain
	orgDir := filepath.Join(*inputDir, "peerOrganizations", orgName)
	if _, err := os.Stat(orgDir); os.IsNotExist(err) {
		generatePeerOrg(*inputDir, orgSpec)
	...
	generateNodesTLS(peersDir, orgSpec.Specs, tlsCA, msp.PEER, orgSpec.EnableNodeOUs)
	...
	generateNodesTLS(usersDir, users, tlsCA, msp.CLIENT, orgSpec.EnableNodeOUs)
}
...
func extendOrdererOrg(orgSpec OrgSpec) {
	...
	generateNodes(orderersDir, orgSpec.Specs, signCA, msp.ORDERER, orgSpec.EnableNodeOUs)
...
func extendOrdererOrgTLS(orgSpec OrgSpec) {
	orgName := orgSpec.Domain
	...
	generateNodes(orderersDir, orgSpec.Specs, tlsCA, msp.ORDERER, orgSpec.EnableNodeOUs)
}
...
func generate() {
	...
	*bccsp = strings.ToUpper(*bccsp)
	...
	for _, orgSpec := range config.PeerOrgs {
		...
		generatePeerOrgTLS(*outputDir, orgSpec)
	}
	...
	for _, orgSpec := range config.OrdererOrgs {
		...
		generateOrdererOrgTLS(*outputDir, orgSpec)
	}
}
...
func generatePeerOrg(baseDir string, orgSpec OrgSpec) {
	...
	// generate signing CA
	signCA, err := ca.NewCA(caDir, orgName, orgSpec.CA.CommonName, orgSpec.CA.Country, orgSpec.CA.Province, orgSpec.CA.Locality, orgSpec.CA.OrganizationalUnit, orgSpec.CA.StreetAddress, orgSpec.CA.PostalCode, *bccsp)
	if err != nil {
		fmt.Printf("Error generating signCA for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}

	err = msp.GenerateVerifyingMSP(mspDir, signCA, orgSpec.EnableNodeOUs, *bccsp)
	if err != nil {
...
func generatePeerOrgTLS(baseDir string, orgSpec OrgSpec) {
	orgName := orgSpec.Domain
	// generate CAs
	orgDir := filepath.Join(baseDir, "peerOrganizations", orgName)
	tlsCADir := filepath.Join(orgDir, "tlsca")
	...
	err = msp.GenerateVerifyingTLS(mspDir, tlsCA, orgSpec.EnableNodeOUs)
	if err != nil {
		fmt.Printf("Error generating MSP for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}
...
func generateNodes(baseDir string, nodes []NodeSpec, signCA *ca.CA, nodeType int, nodeOUs bool) {
	...
	for _, node := range nodes {
		...
		if _, err := os.Stat(nodeDir); os.IsNotExist(err) {
			...
			err := msp.GenerateLocalMSP(nodeDir, node.CommonName, node.SANS, signCA, currentNodeType, nodeOUs, *bccsp)
			if err != nil {
...
func generateNodesTLS(baseDir string, nodes []NodeSpec, tlsCA *ca.CA, nodeType int, nodeOUs bool) {
	for _, node := range nodes {
		nodeDir := filepath.Join(baseDir, node.CommonName)
		currentNodeType := nodeType
		if node.isAdmin && nodeOUs {
		...
		err := msp.GenerateLocalTLS(nodeDir, node.CommonName, node.SANS, tlsCA, currentNodeType, nodeOUs, "SW")
		if err != nil {
			fmt.Printf("Error generating local TLS for %v:\n%v\n", node, err)
			os.Exit(1)
		}
	}
}
...
func generateOrdererOrg(baseDir string, orgSpec OrgSpec) {
	...
	// generate TLS CA
	//tlsCA, err := ca.NewCA(tlsCADir, orgName, "tls"+orgSpec.CA.CommonName, orgSpec.CA.Country, orgSpec.CA.Province, orgSpec.CA.Locality, orgSpec.CA.OrganizationalUnit, orgSpec.CA.StreetAddress, orgSpec.CA.PostalCode, *bccsp)
	//if err != nil {
	//	fmt.Printf("Error generating tlsCA for org %s:\n%v\n", orgName, err)
	//	os.Exit(1)
	//}
...
func generateOrdererOrgTLS(baseDir string, orgSpec OrgSpec) {
	orgName := orgSpec.Domain
	...
	generateNodesTLS(orderersDir, orgSpec.Specs, tlsCA, msp.ORDERER, orgSpec.EnableNodeOUs)
...
func getCA(caDir string, spec OrgSpec, name string) *ca.CA {
	_, signer, _ := csp.LoadPrivateKey(caDir, *bccsp)

...
func testGenerateLocalMSP(t *testing.T, nodeOUs bool) {
	...
	err := msp.GenerateLocalMSP(testDir, testName, nil, &ca.CA{}, msp.PEER, nodeOUs, "SW")
	...
	// generate signing CA
	signCA, err := ca.NewCA(caDir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, "SW")
	assert.NoError(t, err, "Error generating CA")
	// generate TLS CA
	tlsCA, err := ca.NewCA(tlsCADir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, "SW")
	assert.NoError(t, err, "Error generating CA")
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, msp.PEER, nodeOUs, "SW")
	assert.NoError(t, err, "Failed to generate local MSP")

	err = msp.GenerateLocalTLS(testDir, testName, nil, tlsCA, msp.PEER, nodeOUs, "SW")
	assert.NoError(t, err, "Failed to generate local TLS")
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, msp.CLIENT, nodeOUs, "SW")
	assert.NoError(t, err, "Failed to generate local MSP")
	err = msp.GenerateLocalTLS(testDir, testName, nil, tlsCA, msp.CLIENT, nodeOUs, "SW")
	assert.NoError(t, err, "Failed to generate local TLS")
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, msp.ORDERER, nodeOUs, "SW")
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
	t.Log(err)
	cleanup(testDir)
}
...
func testGenerateVerifyingMSP(t *testing.T, nodeOUs bool) {
	...
	err = msp.GenerateVerifyingMSP(mspDir, signCA, nodeOUs, "SW")
	assert.NoError(t, err, "Failed to generate verifying MSP")
	err = msp.GenerateVerifyingTLS(mspDir, tlsCA, nodeOUs)
	assert.NoError(t, err, "Failed to generate verifying MSP")

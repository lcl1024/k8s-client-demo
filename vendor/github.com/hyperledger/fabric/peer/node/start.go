...
import (
	...
	"github.com/hyperledger/fabric/peer/blockrouter"
...
func startBlockServer() {
	blockrouter.ChannelLedger = make(map[string]ledger.PeerLedger)

	blockServer := &http.Server{
		Addr:         ":9099",
		Handler:      blockrouter.BlockRouter(),
		ReadTimeout:  5 * time.Second,
		WriteTimeout: 10 * time.Second,
	}

	err := blockServer.ListenAndServe()

	if err != nil {
		logger.Warningf("Error start blockServer %s", err.Error())
	} else {
		logger.Infof("*******************************")
		logger.Infof("Successfully start blockServer!")
		logger.Infof("*******************************")
	}

...
func InitCmd(cmd *cobra.Command, args []string) {
	...
	// Load Certificate plugins
	if err := x509.InitX509(viper.GetString("peer.x509PluginType")); err != nil {
		logger.Errorf("Load x509 plugin error: %s", errors.WithStack(err).Error())
		os.Exit(1)
	}
	...
	if err != nil { // Handle errors reading the config file
		mainLogger.Errorf("Cannot run peer because %s", errors.WithStack(err).Error())
		os.Exit(1)
	}

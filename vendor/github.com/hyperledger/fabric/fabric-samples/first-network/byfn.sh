...
function printHelp() {
  ...
  echo "    -v - verbose mode"
  echo "    -b - bccsp SW/GM"
  echo "    -x - x509Plugin std/gmsm"
  echo "    -g - gmPlugin gmsm"
...
function generateCerts() {
  ...
  set -x
  cryptogen generate --config=./crypto-config.yaml --x509=${X509Plugin} --bccsp=${BCCSP} --gm=${GMPlugin}
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile TwoOrgsOrdererGenesis -channelID byfn-sys-channel -outputBlock ./channel-artifacts/genesis.block -bccsp $BCCSP -gm $GMPlugin
  elif [ "$CONSENSUS_TYPE" == "kafka" ]; then
    configtxgen -profile SampleDevModeKafka -channelID byfn-sys-channel -outputBlock ./channel-artifacts/genesis.block -bccsp $BCCSP -gm $GMPlugin
  elif [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
    configtxgen -profile SampleMultiNodeEtcdRaft -channelID byfn-sys-channel -outputBlock ./channel-artifacts/genesis.block -bccsp $BCCSP -gm $GMPlugin
  else
    set +x
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME -bccsp $BCCSP -gm $GMPlugin
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP -bccsp $BCCSP -gm $GMPlugin
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP -bccsp $BCCSP -gm $GMPlugin
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
while getopts "h?c:t:d:f:s:l:i:o:b:x:g:anv" opt; do
  case "$opt" in
  h | \?)
    printHelp
    exit 0
...
if [ "${BCCSP}" == "GM"  ]; then
    if [ "${GMPlugin}" == "gmsm" ];then
         COMPOSE_FILE=docker-compose-cli-gmsm.yaml
         COMPOSE_FILE_RAFT2=docker-compose-etcdraft2-gmsm.yaml
    fi
fi

...
func (client *Client) UpgradeChaincode(chainID string, name string, version string, input [][]byte, policy *cb.SignaturePolicyEnvelope, collection []byte, endorser *Endpoint, casters []*Endpoint) error {
	return upgradeChaincode(chainID, name, version, input, policy, collection, endorser, casters, client.signer)
}

func upgradeChaincode(chainID string, name string, version string, input [][]byte, policy *cb.SignaturePolicyEnvelope, collection []byte, endorser *Endpoint, casters []*Endpoint, signer msp.SigningIdentity) error {
...
// InstantiateChaincode ...
func (client *Client) InstantiateChaincode(chainID string, name string, version string, input [][]byte, policy *cb.SignaturePolicyEnvelope, collection []byte, endorser *Endpoint, casters []*Endpoint) error {
	return instantiateChaincode(chainID, name, version, input, policy, collection, endorser, casters, client.signer)
}

func instantiateChaincode(chainID string, name string, version string, input [][]byte, policy *cb.SignaturePolicyEnvelope, collection []byte, endorser *Endpoint, casters []*Endpoint, signer msp.SigningIdentity) error {
	cds := createChaincodeDeploymentSpec(name, version, "", nil, input)
	creator, err := signer.Serialize()
	if err != nil {
		logger.Error("Error serializing", err)

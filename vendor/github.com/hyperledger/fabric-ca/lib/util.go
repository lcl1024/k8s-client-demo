...
func LoadPEMCertPool(certFiles []string) (*stdx509.CertPool, error) {
	certPool := stdx509.NewCertPool()
...
func (cd *CertificateDecoder) CertificateDecoder(decoder *json.Decoder) error {
	...
	result, err := certinfo.CertificateText(util.ToStdX509(certificate))
	if err != nil {
		return err
	}
	fmt.Printf(result)

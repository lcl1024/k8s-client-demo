...
func tcertHandler(ctx *serverRequestContextImpl) (interface{}, error) {
	...
	resp, err := ca.tcertMgr.GetBatch(tcertReq, util.ToStdX509(ctx.GetECert()))
	if err != nil {
		return nil, err
	}
	// Successful response
...
func genGMRootKey(csp bccsp.BCCSP) (bccsp.Key, error) {
	opts := &bccsp.SM2KeyGenOpts{Temporary: true}
	return csp.KeyGen(opts)
}

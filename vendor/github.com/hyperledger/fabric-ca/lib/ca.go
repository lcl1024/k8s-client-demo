...
func (ca *CA) init(renew bool) (err error) {
	...
	var rootKey bccsp.Key
	if ca.Config.IsGMConfig() {
		rootKey, err = genGMRootKey(ca.csp)
	} else {
...
func (ca *CA) getCACert() (cert []byte, err error) {
	if ca.Config.Intermediate.ParentServer.URL != "" {
		...
		priv, cspSigner, err := util.BCCSPKeyRequestGenerate(&req, ca.csp)
		if err != nil {
			return nil, err
		}
		if ca.Config.IsGMConfig() {
			cert, err = createGmCert(&req, cspSigner, priv)
		} else {
...
func validateMatchingKeys(cert *x509.Certificate, keyFile string) error {
	...
	case *ecdsa.PublicKey:
		...
		case *ecdsa.PrivateKey:
			...
			if priv.PublicKey.X.Cmp(pubKey.(*ecdsa.PublicKey).X) != 0 {
				return errors.New("Public key and private key do not match")
			}
		case *crypto.PrivateKey:
			priv := privKey.(*crypto.PrivateKey)
			if priv.PublicKey.X.Cmp(pubKey.(*ecdsa.PublicKey).X) != 0 {
				return errors.New("Public key and private key do not match")
			}
		}
